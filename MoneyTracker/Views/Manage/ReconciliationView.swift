...
struct ReconciliationView: View {
    ...
    private func createAdjustmentTransaction(for account: Account) {
            // The parameters are now strictly ordered to match the database blueprint
            let adjustment = Transaction(
                amount: abs(discrepancy),
                name: "Reconciliation",
                type: discrepancy > 0 ? "income" : "expense",
                timestamp: Date(),
                isAutoGenerated: true, // Tagging this as a system action!
                account: account,
                destinationAccount: nil,
                category: nil,
                note: "System generated adjustment"
            )
